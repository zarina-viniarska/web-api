  ...
  return (
    <TableHead>
      <TableRow>
        ...
        {user.role === "Administrators" && <TableCell></TableCell>}
      </TableRow>
    </TableHead>
  );
}
...
const Courses: React.FC = () => {
  ...
  const { allCourses } = useTypedSelector((store) => store.CourseReducer);
  const { user } = useTypedSelector((store) => store.UserReducer);
  ...
  const handleSelectAllClick = (event: React.ChangeEvent<HTMLInputElement>) => {
    if (event.target.checked) {
      const newSelected = allCourses.map((n: any) => n.title);
  ...
  const handleEditClick = (e: any, course: any) => {
    e.stopPropagation();
    // SelectUser(user.id);
    console.log("Edit click" + course);
    ...
    <Box sx={{ width: "100%" }}>
      {user.role === "Administrators" && (
        <Box
          ...
          alignItems="flex-end"
        >
          <Button variant="contained" href="add-course">
            Add new course
          </Button>
        </Box>
      )}
