...
const Category = {
  ...
  Insert: (category: any) => request.post("/create", category),
  Update: (category: any) => request.post("/update", category),
...
export async function Insert(category: any) {
  const data = await Category.Insert(category)
    .then((response) => {
      return { response };
    })
    .catch((error) => {
